...
class CommandGenerator {
	...
	def createAttributeDeclarations(BusinessMethod m) {
		'''
			«FOR p : m.methodParameters»
				private «p.variableType» «p.variableName»;
			«ENDFOR»
	...
	def createDefaultConstructor(BusinessMethod m) {
		'''
			public «StringUtils.toUpperFirst(m.methodName)»Command(«FOR p : m.methodParameters»«p.variableType» «p.variableName»«IF !p.equals(m.methodParameters.get(m.methodParameters.size-1))», «ENDIF»«ENDFOR») {
				«FOR p : m.methodParameters»
